...
/** true si hay servicio de envío configurado; si no, el formulario abre el correo del usuario. */
...
export function mailtoHref(to: string, m: Pick<ContactMessage, 'name' | 'subject' | 'message'>): string {
  const body = m.name ? `${m.message}\n\n— ${m.name}` : m.message;
