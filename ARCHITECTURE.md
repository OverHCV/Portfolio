...
- **Chips (Acto 5):** cajas de hover invisibles sobre cada chip (`Buildings.tsx`); clic → `focus = { kind: 'project', id }` (soltar tras arrastrar no cuenta: `DRAG_SLOP`). La tarjeta breve (`nearProject`) es la del chip cuyo centro en pantalla está a menos de `PROXIMITY_PX` del cursor; sin mouse (táctil), la del chip más cercano al centro de la pantalla. Ese chip se levanta y sus filos y calles brillan (`uLift`/`uGlow`, filo con el acento `uAccent`). La tarjeta es una burbuja que sale del chip: la escena proyecta su caja a pantalla cada frame (`Act5City/anchor.ts`) y `ProjectBubble` la coloca arriba, al lado o abajo (donde quepa), con cola y línea guía. Solo informa: no recibe el puntero (nunca tapa otros chips) y marca «clic en el chip para abrir», que se enciende con el cursor sobre el chip. Las capturas (`ProjectShots`) son un carrusel infinito que avanza solo, en la burbuja y en el panel. `CityOverlay` expone además la lista de proyectos como botones para teclado.
